def frieze_mat2(quiddity_row=(2,1,4,2,3),inputcol=1,inputrow=8,flag_rectangle=True):
    """
    ...
    so that the quiddity row is at "frieze row"=2
    the row of 1s is at "frieze row" = 1
    and the row of 0s is at “frieze row” = 0
    """
    n=len(quiddity_row)
    m=dict()
    ...
    for col in range(inputrowplusextra-1):
	...
	for col in range(inputcol-1,inputrowplusextra-friezerow):
            ...
            if type(m[(i,j)]) == type(sqrt(2)):
                m[(i,j)] = m[(i,j)].full_simplify()
            #print m[(i,j)]
        if m[(inputcol+friezerow,inputcol)]<1:
            break
        #print '\n'
    return m
